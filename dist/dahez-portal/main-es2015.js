...
class AuthInterceptor {
    constructor(authService, errorService, router) {
     ...
     */
    handleAuthRequest(request, token) {
    ...
    intercept(request, next) {
        const token = localStorage.getItem(_util_constants__WEBPACK_IMPORTED_MODULE_1__["Constants"].TOKEN_NAME);
...
let AuthService = class AuthService {
    ...
    isLoged() {
        const token = localStorage.getItem(_util_constants__WEBPACK_IMPORTED_MODULE_2__["Constants"].TOKEN_NAME);
        const expireTime = localStorage.getItem(_util_constants__WEBPACK_IMPORTED_MODULE_2__["Constants"].TOKEN_EXPIRE_NAME);
    ...
    getUserInfo() {
        return this.http.get('oauth/User')
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["tap"])(user => this.setUser(user)));
    }
    /**
    ...
    getHeaders() {
        const token = localStorage.getItem(_util_constants__WEBPACK_IMPORTED_MODULE_2__["Constants"].TOKEN_NAME);
        const headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpHeaders"]({ 'Authorization': `Bearer ${token}` });
        return headers;
    }
    /**
    ...
    getToken(user, password) {
        ...
        return this.http.post('oauth/token', params)
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["tap"])(token => this.saveToken(token)));
    }
    /**
...
let PermissionDirective = class PermissionDirective {
    ...
    validateUserCredentials(user, roles) {
...
let ErrorService = class ErrorService {
    ...
    handleBlobError(error) {
        return (() => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () { return yield (new Response(error.error).text()); }))()
